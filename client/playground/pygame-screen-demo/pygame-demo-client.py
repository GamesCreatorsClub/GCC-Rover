...
def connected():
    ...
    pyros.agent.init(pyros.client, "pygame-demo-agent.py")
    print("Done.")
...
def stop():
    global running
    pyros.publish("pygame-demo/command", "stop")
    running = False


def start():
    global running
    pyros.publish("pygame-demo/command", "start")
    running = True
...
pyros.init("pygame-demo-#", unique=True, onConnected=connected, host=pyros.gcc.getHost(), port=pyros.gcc.getPort(), waitToConnect=False)
