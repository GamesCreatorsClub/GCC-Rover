...
class CanyonsOfMars:
    ...
    def handleHeading(self, topic, message, groups):
        ...
        if self.start_heading == -1:
            self.start_heading = orientation
        else:
            orientation -= self.start_heading
        if orientation < 0:
            orientation += 360

        self.heading = orientation
        self.heading_label.setText("{:3.2f}".format(self.heading))
...
class MazeCornerComponent(gccui.components.CardsCollection):
    def __init__(self, rect):
        super(MazeCornerComponent, self).__init__(rect)
...
def initGraphics(screens):
    ...
    canyonsOfMars.heading_label = uiFactory.label(Rect(630, 80, 70, 20), "", h_alignment=gccui.ALIGNMENT.RIGHT)
    ...
    statusComponents.addComponent(uiFactory.label(Rect(630, 80, 30, 20), "H:"))
    statusComponents.addComponent(canyonsOfMars.heading_label)
