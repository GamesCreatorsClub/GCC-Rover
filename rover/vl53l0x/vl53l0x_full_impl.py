...
def initVL53L0X():
    ...
    i2cBus.write_i2c_block_data(I2C_ADDRESS, VL53L0X_REG_GLOBAL_CONFIG_SPAD_ENABLES_REF_0, ref_spad_map)
...
def startContinuous(period_ms):
    ...
    if period_ms != 0:
        ...
        osc_calibrate_val = _readReg16Bit(VL53L0X_REG_OSC_CALIBRATE_VAL)

        if osc_calibrate_val != 0:
            period_ms *= osc_calibrate_val

        _writeReg32Bit(VL53L0X_REG_SYSTEM_INTERMEASUREMENT_PERIOD, period_ms)
...
def readRangeContinuousMillimeters():
    ...
    resultRange = _readReg16Bit(VL53L0X_REG_RESULT_RANGE_STATUS + 10)
...
def readRangeContinuousMillimetersFastFail():
    global didTimeout

    if (i2cBus.read_byte_data(I2C_ADDRESS, VL53L0X_REG_RESULT_INTERRUPT_STATUS) & 0x07) == 0:
        return -1
...
def setMeasurementTimingBudget(budget_us):
    ...
    if enables["final_range"]:
        ...
        _writeReg16Bit(VL53L0X_REG_FINAL_RANGE_CONFIG_TIMEOUT_MACROP_HI, encodeTimeout(final_range_timeout_mclks))
...
def _getVcselPulsePeriod(tpe):
    if tpe == VcselPeriodPreRange:
        return _decodeVcselPeriod(i2cBus.read_byte_data(I2C_ADDRESS, VL53L0X_REG_PRE_RANGE_CONFIG_VCSEL_PERIOD))
    elif tpe == VcselPeriodFinalRange:
        return _decodeVcselPeriod(i2cBus.read_byte_data(I2C_ADDRESS, VL53L0X_REG_FINAL_RANGE_CONFIG_VCSEL_PERIOD))
    else:
        return 255
...
def getSequenceStepTimeouts():
    timeouts["pre_range_vcsel_period_pclks"] = _getVcselPulsePeriod(VcselPeriodPreRange)

    timeouts["msrc_dss_tcc_mclks"] = (i2cBus.read_byte_data(I2C_ADDRESS, VL53L0X_REG_MSRC_CONFIG_TIMEOUT_MACROP) + 1) & 0xFF
    timeouts["msrc_dss_tcc_us"] = _timeoutMclksToMicroseconds(timeouts["msrc_dss_tcc_mclks"], timeouts["pre_range_vcsel_period_pclks"])

    timeouts["pre_range_mclks"] = _decodeTimeout(_readReg16Bit(VL53L0X_REG_PRE_RANGE_CONFIG_TIMEOUT_MACROP_HI))
    timeouts["pre_range_us"] = _timeoutMclksToMicroseconds(timeouts["pre_range_mclks"], timeouts["pre_range_vcsel_period_pclks"])

    timeouts["final_range_vcsel_period_pclks"] = _getVcselPulsePeriod(VcselPeriodFinalRange)

    timeouts["final_range_mclks"] = _decodeTimeout(_readReg16Bit(VL53L0X_REG_FINAL_RANGE_CONFIG_TIMEOUT_MACROP_HI))
...
def setSignalRateLimit(limit_Mcps):
    ...
    _writeReg16Bit(VL53L0X_REG_FINAL_RANGE_CONFIG_MIN_COUNT_RATE_RTN_LIMIT, int(limit_Mcps * (1 << 7)))
    return True


def _writeReg16Bit(reg, value):
    block = [(int(value) >> 8) & 0xFF, int(value) & 0xFF]
    i2cBus.write_i2c_block_data(I2C_ADDRESS, reg, block)


def _writeReg32Bit(reg, value):
    block = [(int(value) >> 24) & 0xFF, (int(value) >> 16) & 0xFF, (int(value) >> 8) & 0xFF, int(value) & 0xFF]
    i2cBus.write_i2c_block_data(I2C_ADDRESS, reg, block)


def _readReg16Bit(reg):
